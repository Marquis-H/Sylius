...
final class MainMenuBuilder
{
     ...
     * @param ItemInterface $menu
     */
    public function addMemberSubMenu(ItemInterface $menu)
    {
        $member = $menu
            ->addChild('member')
            ->setLabel('会员');
        $member
            ->addChild('member_category', ['route' => 'sylius_admin_member_category_index'])
            ->setLabel('会员卡')
            ->setLabelAttribute('icon', 'credit card alternative');

        $member
            ->addChild('member_extend', ['route' => 'sylius_admin_member_extend_index'])
            ->setLabel('发放')
            ->setLabelAttribute('icon', 'credit card alternative');

        $member
            ->addChild('member_record', ['route' => 'sylius_admin_member_record_index'])
            ->setLabel('使用记录')
            ->setLabelAttribute('icon', 'credit card alternative');
    }
